...
users_check = []
try:
    with open('usersbase.csv') as archivo:
        leer_csv = csv.reader(archivo)
        for linea in leer_csv:
            users_check.append(linea[0])
except FileNotFoundError:
    print('Error de CSV de usersbase')

try:
    with open('basedatos.csv') as archivo:
        pass
except FileNotFoundError:
        ...
        try:
            with open('usersbase.csv') as archivo:
                try:
                    ...
                    return 'usuario de usersbase.csv invalido'        
        except FileNotFoundError:
            return 'No se encuentra el archivo de usersbase'
    return render_template('Ingreso.html', form=fomu_log, username=session.get('usuarioLoggeado'))

#Solo se vera la BD si esta ingresado al Sistema.
@app.route('/basededatos', methods=['GET', 'POST'])
def bdatos():
    if 'usuarioLoggeado' in session:
        try:
            with open('basedatos.csv', 'r') as archivo:
            ...
            return 'No se encuentra la base de datos de FarmaMura'
    return render_template('sign_off.html')
...
def ulti_ventas():
    if 'usuarioLoggeado' in session:
        try:
            with open('ultimas_ventas.csv', 'r') as archivo:
                datalines = csv.reader(archivo)                
                titulos = next(datalines)                                
                return render_template('ultimas_ventas.html', cabeza=titulos, cuerpo=datalines, username=session.get('usuarioLoggeado'))
        except FileNotFoundError:
            return 'No se encuentra la base de datos de FarmaMura'
    return render_template('sign_off.html')

#Solo se vera si es que se ingreso correctamente.
...
def register():
    ...
    if form_registro.validate_on_submit():
        if form_registro.pass1.data == form_registro.pass2.data:
            try:
                with open('usersbase.csv', 'a') as archivo:
